...
for column in high_cardinality_cols:
    ...
    # Join frequency back to main DataFrame
    train_df_encoded = train_df_encoded.join(freq_df, on=column, how="left")
    val_df_encoded = val_df_encoded.join(freq_df, on=column, how="left")
    test_df_encoded = test_df_encoded.join(freq_df, on=column, how="left")
...
train_df_encoded.select(encoded_cols).display()
...
duplicate_indexes_train.display()
...
duplicate_rows.select(columns_to_show).display()
...
for col_name in cleaned_train_df.columns:
    ...
    display(cleaned_train_df.select(col_name).distinct())
...
display(cleaned_train_df[['DEPARTURE_TIME', 'AIR_TIME', 'CROSSES_MIDNIGHT_FLAG']])

# COMMAND ----------

#rows in which departure delay is negative, the flights departure before their scheduled time
display(cleaned_train_df.filter(cleaned_train_df.DEPARTURE_DELAY < 0))
...
display(early_departures_without_condition) #it is the same number of rows as with the condition of the flights don't cross midnight
...
display(cleaned_train_df.filter(
...
display(invalid_flights_df.select("AIR_TIME", "DEPARTURE_TIME").distinct())
...
display(match_arrival[['SCHEDULED_ARRIVAL_min', 'ARRIVAL_DELAY', 'ARRIVAL_TIME_min', 'CROSSES_MIDNIGHT_FLAG']])
match_arrival.count()
...
#drop irrelevant columns created for the purpose of checking incoherencies
cleaned_train_df = cleaned_train_df.drop('TOTAL_ARRIVAL_UNTIL_MIDNIGHT', 'MINUTES_LEFT_AFTER_MIDNIGHT')
