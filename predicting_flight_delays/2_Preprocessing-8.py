...
# MAGIC %md
# MAGIC ## 2.10.2 Columns and datatypes
...
# MAGIC %md
# MAGIC **Conclusion**: No missing values

# COMMAND ----------

# MAGIC %md
# MAGIC ## 2.10.4 Unique values
...
# MAGIC %md
# MAGIC ## 2.10.5 Time columns

# COMMAND ----------

# MAGIC %md
# MAGIC ### Schedule vs Actual Departure Time
...
# MAGIC %md
# MAGIC We verified that the feature DEPARTURE_TIME aligns with the sum of SCHEDULED_DEPARTURE and DEPARTURE_DELAY. This confirms that the delay information is correctly encoded, and no mismatches were detected.

# COMMAND ----------

# MAGIC %md
# MAGIC ### Consistency Between Scheduled Times (Departure and Arrival)
...
# MAGIC %md
# MAGIC The discrepancies observed between the expected and scheduled arrival times are likely due to differences in time zones between departure and arrival airports. This is supported by the fact that many mismatches consistently differ by 60-minute intervals, which aligns with typical time zone offsets.
...
# MAGIC %md
# MAGIC ABQ_MCI
# MAGIC ( Albuquerque International Sunport - Kansas City International Airport )
# MAGIC <br>
# MAGIC Time Zone Difference: +1 (= 60 minutes)
# MAGIC
# MAGIC ABQ_DAL
# MAGIC ( Albuquerque International Sunport - Dallas Love Field )
# MAGIC <br>
# MAGIC Time Zone Difference: +1 (= 60 minutes)
# MAGIC
# MAGIC ABQ_LAX
# MAGIC ( Albuquerque International Sunport - Los Angeles International Airport )
# MAGIC <br>
# MAGIC Time Zone Difference: -1 (= -60 minutes)
# MAGIC
# MAGIC <br>
# MAGIC This discrepancie is due to time zone differences, it isn't an incoherence in our dataset.
...
cleaned_train_df = cleaned_train_df.withColumn(
    ...
    col('SCHEDULED_DEPARTURE_min') + col('SCHEDULED_TIME')
)
...
cleaned_train_df[['SCHEDULED_DEPARTURE_min', 'SCHEDULED_TIME', 'CROSSES_MIDNIGHT_FLAG']].limit(10).display()
...
def cap_delay(df):
    return df.withColumn(
        "DEPARTURE_DELAY",
        when(col("DEPARTURE_DELAY") < -10, -10)
        .otherwise(col("DEPARTURE_DELAY"))
    )

cleaned_train_df = cap_delay(cleaned_train_df)
cleaned_val_df = cap_delay(cleaned_val_df)
cleaned_test_df = cap_delay(cleaned_test_df)
...
invalid_flights_df.select("SCHEDULED_TIME", "SCHEDULED_DEPARTURE_min").distinct().limit(10).display()
